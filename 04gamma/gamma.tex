...
Therefore, not only the tree-level transition $\bquarkbar\to\uquarkbar\,[\uquark\dquarkbar]$ but also the transition $\bquarkbar\to\dquarkbar\,[{}\dquark\dquarkbar]$ via  the gluonic-penguin is possible beside the spectator \squark-quark (see \cref{fig:Bs2RhoKS}).
\begin{figure}[tbp]
	\centering
...
Furthermore the \CP coefficient \Cf contains the ratio $r_{\kern -1.0pt \B}$, while via the coefficients \Sf, \Sfbar, $A_{\kern -1.5pt f}^{\DG}$, $A_{\kern 1.5pt\overline{\kern -1.5pt f\kern 1.5pt}}^{\DG}$ the phase information are measurable.
...
In \cref{fig:feynmanGammaLoops}, the dominant $\bquarkbar\!\to\uquarkbar\,[\uquark\dquarkbar]$ processes are shown, which leads to the transition amplitude
\begin{equation}
...
\begin{equation}
\begin{aligned}
C_{\kern -1.5pt f}^{\Bd\!\to\pip\pim}&=-\left[\frac{2d\sin\!\left(\theta\right)\sin\!\left(\gamma\right)}{1-2d\cos\!\left(\theta\right)\cos\!\left(\gamma\right)+d^2}\right]\,,\\
S_{\kern -1.5pt f}^{\Bd\!\to\pip\pim}&=\left[\frac{\sin\!\left(2\beta+2\gamma\right)-2d\cos\!\left(\theta\right)\sin\!\left(2\beta+\gamma\right)+d^2\sin\!\left(2\beta\right)}{1-2d\cos\!\left(\theta\right)\cos\!\left(\gamma\right)+d^2}\right]\,.
\end{aligned}
\end{equation}
...
The \CP coefficient $A_{\kern -1.5pt f}^{\DG}$ is neglected as the decay width difference for \Bd mesons is expected to be negligibly small.

Equivalently to this, the \CP coefficients for $\Bs\!\to\Kp\Km$ decays can be calculated as
\begin{equation}
\begin{aligned}
C_{\kern -1.5pt f}^{\Bs\!\to\Kp\Km}&\!=\!\left[\frac{2\tilde{d'}\!\sin\!\left(\theta'\right)\sin\!\left(\gamma\right)}{1-2\tilde{d'}\!\cos\!\left(\theta'\right)\cos\!\left(\gamma\right)+\tilde{d'}^2}\right]\,,\\
S_{\kern -1.5pt f}^{\Bs\!\to\Kp\Km}&\!=\!\left[\frac{\sin\!\left(\phi_\squark+2\gamma\right)-2\tilde{d'}\!\cos\!\left(\theta\right)\sin\!\left(\phi_\squark+\gamma\right)+\tilde{d'}{}^2\!\sin\!\left(\phi_\squark\right)}{1-2\tilde{d'}\!\cos\!\left(\theta'\right)\cos\!\left(\gamma\right)+\tilde{d'}{}^2}\right]\,,\\
A_{\kern -1.5pt f}^{\DG, \Bs\!\to\Kp\Km}&\!=\!-\left[\frac{\cos\!\left(\phi_\squark+2\gamma\right)+2\tilde{d'}\!\cos\!\left(\theta'\right)\cos\!\left(\phi_\squark+\gamma\right)+\tilde{d'}{}^2\cos\!\left(\phi_\squark\right)}{1-2\tilde{d'}\!\cos\!\left(\theta'\right)\cos\!\left(\gamma\right)+\tilde{d'}{}^2}\right]
\end{aligned}
\end{equation}
...
The first approach~\cite{GammaInLoops_Fleischer} uses an external input for the phase $\phi_s$, so that the four unknows $d$, $\theta$, $\beta$ and $\gamma$ can be determined using only the four observables $C_{\kern -1.5pt f}^{B\!\to hh}$, and $S_{\kern -1.5pt f}^{B\!\to hh}$.
As shown in Ref.~\cite{GammaInLoops_Fleischer}, the parameters $d$ and $\theta$ can be expressed as functions of $\gamma$ and the \CP coefficients $\smash{C_{\kern -1.5pt f}^{\Bs\!\to\Kp\Km}}$ and $\smash{S_{\kern -1.5pt f}^{\Bs\!\to\Kp\Km}}$.
Inserting this into $C_{\kern -1.5pt f}^{\Bd\!\to\pip\pim}$ then allows to determine $\gamma$ up to a two-fold ambiguity as shown in \cref{fig:gamma_beta_fleischer}.
\begin{figure}[tbp]
	\centering
	...
	\caption{Dependence of $C_{\kern -1.5pt f}^{\Bd\!\to\pip\pim}$ ($A^{\text{dir}}$) on $\gamma$ (left) and $S_{\kern -1.5pt f}^{\Bd\!\to\pip\pim}$ ($A^{\text{mix}}$) on $\beta$ (right) for a specific example as given in Ref.~\cite{GammaInLoops_Fleischer}.}
	\label{fig:gamma_beta_fleischer}
\end{figure}
Using these two solutions for $\gamma$ and inserting them together with the obtained expressions for $d$ and $\theta$ into $S_{\kern -1.5pt f}^{\Bd\!\to\pip\pim}$ gives another twofold ambiguity for $\beta$ (see \cref{fig:gamma_beta_fleischer}).
Hereby, for both ambiguities the assumption is made that $\gamma\in[0, 180]\si{\degree}$ and $\beta\in[0, 180]\si{\degree}$.
If additionally the phase $\beta$ is used, this ambiguities can be resolved by expressing $d$ and $\theta$ as functions of $\gamma$ and the \CP coefficients $C_{\kern -1.5pt f}^{\Bd\!\to\pip\pim}$ and $S_{\kern -1.5pt f}^{\Bd\!\to\pip\pim}$ and consequently fixing the contours in the $\gamma-d$ plane.
...
\begin{equation}
\gamma=\big(74.0^{+5.0}_{-5.8}\big)\!\si{\degree}.\label{eq:gammaResult_Trees}
\end{equation}
...
More precisely, the amplitudes of the $\bquark\!\to\cquark\,[\uquarkbar\dquark]$ and $\bquark\!\to\uquark\,[\cquarkbar\dquark]$ transitions interfer.
\begin{figure}[tbp]
	\centering
...
\begin{align}
A\bigs(\!\Bz\!\to\Dm\pip\bigs)&=\Vud\Vcbst\times Me^{i\Delta}\,,\\
A\bigs(\!\Bzb\!\to\Dm\pip\bigs)&=\Vub\Vcdst\times\kern 2.0pt\overline{\kern -2.0pt M\kern -0.0pt}\,e^{i\kern 2.5pt\overline{\kern -2.5pt \Delta\kern -0.0pt}}\,,\\
A\bigs(\!\Bz\!\to\Dp\pim\bigs)&=\Vubst\Vcd\times\kern 2.0pt\overline{\kern -2.0pt M\kern -0.0pt}\,e^{i\kern 2.5pt\overline{\kern -2.5pt \Delta\kern -0.0pt}}\,,\\
A\bigs(\!\Bzb\!\to\Dp\pim\bigs)&=\Vudst\Vcb\times Me^{i\Delta}\,.
\end{align}
...
\begin{align}
\Lf&=\frac{q}{p}\frac{A\bigs(\!\Bzb\!\to\Dm\pip\bigs)}{A\bigs(\!\Bz\!\to\Dm\pip\bigs)}=-\frac{\Vtbst\Vtd}{\Vtb\Vtdst}\frac{\Vub\Vcdst}{\Vud\Vcbst}\frac{\kern 2.0pt\overline{\kern -2.0pt M\kern -0.0pt}}{M}e^{i\delta}\,,\\
\Lfbar&=\frac{q}{p}\frac{A\bigs(\!\Bzb\!\to\Dp\pim\bigs)}{A\bigs(\!\Bz\!\to\Dp\pim\bigs)}=-\frac{\Vtbst\Vtd}{\Vtb\Vtdst}\frac{\Vudst\Vcb}{\Vubst\Vcd}\frac{M}{\kern 2.0pt\overline{\kern -2.0pt M\kern -0.0pt}}e^{-i\delta}
\end{align}
where the abbreviation $\delta=\kern 2.5pt\overline{\kern -2.5pt \Delta\kern -0.0pt}-\Delta$ was applied.
...
\begin{align}
\Lf&=-\left|\frac{\Vub\Vcd}{\Vud\Vcb}\right|\frac{\kern 2.0pt\overline{\kern -2.0pt M\kern -0.0pt}}{M}e^{-i\left(2\beta+\gamma-\delta\right)}=-re^{-i\left(2\beta+\gamma-\delta\right)}\,,\\
\Lfbar&=-\left|\frac{\Vudst\Vcbst}{\Vubst\Vcdst}\right|\frac{M}{\kern 2.0pt\overline{\kern -2.0pt M\kern -0.0pt}}e^{-i\left(2\beta+\gamma+\delta\right)}=-\frac{1}{r}e^{-i\left(2\beta+\gamma+\delta\right)}.
\end{align}
...
\begin{align}
\Gamma\bigs(\!\Bz(t)\!\to\Dm\pip\bigs)&= \frac{A}{2}e^{\Gamma t}\bigs[1-\Sf\sin\!\left(\dm t\right)+\Cf\cos\!\left(\dm t\right)\bigs]\label{eq:DecRateB2Dmpip}\,,\\
\Gamma\bigs(\!\Bzb(t)\!\to\Dm\pip\bigs)&= \frac{A}{2}e^{\Gamma t}\bigs[1+\Sf\sin\!\left(\dm t\right)-\Cf\cos\!\left(\dm t\right)\bigs]\label{eq:DecRateBb2Dmpip}\,,\\
\Gamma\bigs(\!\Bz(t)\!\to\Dp\pim\bigs)&= \frac{\overline{\kern -1.0pt A\kern -1.0pt}}{2}e^{\Gamma t}\bigs[1-\Sfbar\!\sin\!\left(\dm t\right)+\Cfbar\!\cos\!\left(\dm t\right)\bigs]\label{eq:DecRateB2Dppim}\,,\\
\Gamma\bigs(\!\Bzb(t)\!\to\Dp\pim\bigs)&= \frac{\overline{\kern -1.0pt A\kern -1.0pt}}{2}e^{\Gamma t}\bigs[1+\Sfbar\!\sin\!\left(\dm t\right)-\Cfbar\!\cos\!\left(\dm t\right)\bigs]\label{eq:DecRateBb2Dppim}\,,
\end{align}
...
From $\DG=0$ follows, that the parameters $A_{\kern -1.5pt f}^{\DG}$ and $A_{\kern 1.0pt\overline{\kern -2.5pt f\kern 1.0pt}}^{\DG}$ do not contribute to the decay widths while the remaining \CP parameters
\begin{align}
...
However only the contribution to the $\bquark\!\to\cquark\,[\uquarkbar\dquark]$ transition needs to be taken into account as effects of $\bquark\!\to\uquark\,[\cquarkbar\dquark]$-transitions are suppressed by $\lambda^2$ and can be neglected.
