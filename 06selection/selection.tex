...
These requirement together with all cuts on single particles given in \cref{tab:stripping} aim to select \Bz decays into a charged \D meson including \Dst mesons and pion, where the \D mesons decays into three hadrons from combinatorial backgrounds candidates.
...
\end{figure}
For this analysis, no specifc requirements at the L0 level are needed.
...
As this approach cannot be applied to data, a criterion is developed requiring for each (\Bz,\ac{PV})-candidate the impact parameter $\chi^2_{\text{DTF,PV}}$ ($\text{IP}\chi^2$) with any other \ac{PV} in the event (denoted as $\text{MinIP}\chi^2$) to be larger than a certain value.
...
\begin{figure}[tbp]
    ...
    \caption{The left plots show the $\text{MinIP}\chi^2$ variable with the chosen cut point at \num{16.5} in a narrow range.
    For larger values of $\text{MinIP}\chi^2$ the distribution remains flat.
    The right plot shows the decay time distribution of simulated \BdToDpi candidates, wheighted with an exponential using the true lifetime of the \Bz candidates after applying the cut on $\text{MinIP}\chi^2$ (right).
    The expected decay-time acceptance distribution at \lhcb is visible.}
    \label{fig:WrongPVData}
\end{figure}

In contrast the \ac{PV} could also be chosen using some \enquote{best} \ac{PV} criterion, \eg choosing the \ac{PV} with the smallest $\text{IP}\chi^2$ with respect to the \Bz candidate.
