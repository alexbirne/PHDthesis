...
Using the \dllkpi of the bachelor pion allows to split the data sample into the two samples, referred to as \emph{pion} and \emph{kaon} sample: The \emph{pion} sample has $\dllkpi\leq5.0$ and the \emph{kaon} sample $\dllkpi>5.0$. This distinction is useful when separating \BdToDpi from $\Bd\!\to\Dm\Kp$ candidates as no dedicated selection cut is needed.
...
To determine \emph{sWeights}~\cite{Pivk:2004ty} the invariant \Bz mass is fitted with a two-stage strategy:
