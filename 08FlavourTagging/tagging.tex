...
This makes small effects that were hidden in previous \CP-violation analyses by the statistical uncertainties significant.
...
A qualitative explanation is given in \cref{fig:FTstrategy}.
A more quantitative validation was done by a collaborator on pseudoexperiments:
...
\end{table}
For each tagging-particle candidate passing the requirements shown in \cref{tab:SSPionselectionCuts}, the \B-meson gets a preliminary tag decision assigned, corresponding to the charge of the tagging particle ($\pip\rightarrow d=+1$, $\pim\rightarrow d=-1$).
...
With a fit, binned in the BDT output $\alpha^{\text{SS}\proton}_\text{BDT}$, firstly the functional relation between the mistag and $\alpha^{\text{SS}\proton}_\text{BDT}$ is determined; a $3^{\text{rd}}$ order polynomial as described in \cref{eq:tranformfunc} describes the data well again.
An unbinned fit, in which the mistag is parametrised directly in the PDF, then provides the final transformation parameters, which can be found in \cref{tab:transformSSProton}.
\begin{table}[tbp]
	\centering
...
\begin{table}[tbp]
        ...
        \label{tab:GOFSS}
\end{table}
Since the deviance $G^2$ and the le Cessie-van Houwelingen-Copas-Hosmer metric ($S$) seem to prefer the more simple, while the Pearson $\chi^2$ and the Cressie-Read ($CR$) metric prefer the more complicated model, but are assumed to fit the data equally well and the more simple is chosen (all metrics are described in Ref.~\cite{GOFmetric}).
...
The calibration model is a GLM with a natural spline function as \emph{basis function} and also the modified logistic function from \cref{eq:modlink} as \emph{link function}.
