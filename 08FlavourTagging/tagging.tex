...
In order to do this the adopted model is a GLM with a $1^{text{st}}$ order polynomial as the \emph{basis function} and the modified logistic function from \cref{eq:modlink} as \emph{link function}.
...
\begin{table}[tbp]
        ...
        \label{tab:GOFOS}
\end{table}
The Calibration is then performed using the \emph{sWeights} together with the weights to match calculated before such that the $\Bu\!\to\Dz\pip$ distributions matcht the \BdToDpi ones, giving the calibration parameters shown in \cref{tab:CalibOS}.
