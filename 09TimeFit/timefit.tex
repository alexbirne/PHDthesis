...
Section \ref{sec:resolution} and \ref{sec:acceptance} describe the parameterization of the decay time resolution and acceptance before the extraction of the \CP parameters is presented in \cref{sec:ExtractCPobs}.
Following this fit needs to be validated.
This can be achieved by first comparing other results from the fit than the \CP parameters with references.
...
Last the fit to extract the CP parameters is repeated on different subsamples of the data set (\cref{sec:valOnSubSample}) and the entire strategy is also checked on simulated events (\cref{sec:valOnSim}).

\section{Fit to data}

In the following the last preparatpory studies and the decay-time fit to extract \Sf and \Sfbar are presented.
...
The upper limit was set to a value such that the statistics is already too small that an enlarged range would no longer be useful add sensitivity on the \CP parameters.
...
For this second asymmetries in addition to the fit projection of the nominal fit also the fit projection of an alternative fit assuming no \CP violation, \ie $\Sf=-\Sfbar$, is shown.
\begin{figure}[tbp]
    \centering
    ...
    \caption{Top: Decay-time dependetn signal yield asymmetry for the \Dm\pip (left) and the \Dp\pim (right) finalstate.
    The solid curve represent the projection of the fitted PDF.
    Bottom: Decay-time dependetn signal yield asymmetry for the favoured (left) and the suppressed (right) transitions.
    The asymmetries are defined in \cref{eq:AsymeSuppFav}.
    The blue solid curve is the porjection of the fitted PDF from the nominal fit, the red dotted curve shows the projection of a second fit imposing no \CP violation.}
...
The average dilution is \SI{9.53\pm0.03}{\percent} and \SI{2.789\pm0.009}{\percent} for the OS and SS taggers, repsectively.
This leads to an overall average dilution of \SI{6.55\pm0.02}{\percent}.
Including also the untagged candidates which were removed in \cref{ch:massfit}, the total effective tagging efficiency can be calculated to be \SI{5.59\pm0.01}{\percent}.
...
The obtained calibration parameters are compared to those computed on the control channels $\Bu\!to\Dz\pip$ and $\Bz\!\to\jpsi\Kstarz$ for the OS and SS, respectively.
In \cref{tab:taggingCalibCompare} the parameters from the decay-time fit in \BdToDpi are listed together with the deviations from the calibration parameters given in \cref{tab:CalibSS} and \cref{tab:CalibOS}.
The largest deviation can be found for $\Delta p_{X}$ and $\Delta p_{Y}$ being of the size of two standard deviations.
Additionally an overall discrepancy taking into account the correlations between the parameters is calculated yielding $0.91\sigma$ and $0.29\sigma$ for the OS and SS taggers, respectively.
\begin{table}[tbp]
	\centering
	...
	\begin{tabular}{c|SS|SS}
		...
		\midrule
		{$p_0$} 		& -0.152\pm0.021 	& 0.021  & -0.041\pm0.021 & 3\\
		{$p_1$} 		& -0.035\pm0.024 	& 0.021  & -0.012\pm0.022 & 2\\
		{$p_2$} 		& -0.007\pm0.009 	& 0.0022 & {-} 			  & {-}\\
		{$p_3$} 		& -0.32\pm0.11 		& 0.0040 & {-}			  & {-}\\
		{$p_4$} 		& -0.47\pm0.49 		& 0.0028 & {-}			  & {-}\\
		{$\Delta p_0$} 	& -0.079\pm0.049 	& 0.021  & -0.085\pm0.044 & 1\\
		{$\Delta p_1$} 	& 0.140\pm0.036 	& 0.021  & 0.042\pm0.033  & 0.5\\
		{$\Delta p_2$} 	& -0.024\pm0.013 	& 0.0022 & {-} 			  & {-}\\
		{$\Delta p_3$} 	& -0.26\pm0.16 		& 0.0040 & {-} 			  & {-} \\
		{$\Delta p_4$} 	& -0.52\pm0.71 		& 0.0028 & {-} 			  & {-} \\
		\bottomrule
	\end{tabular}
...
As shown in \cref{fig:corrPlots} both do not show any unexpected behaviour but only the expected Gaussian ellipses, indicating that the corresponding uncertainties are well understood.
\begin{figure}[tbp]
    \centering
...
As mentiond before the handling of candidates with a mistag close to \num{0.5} is important both in case of a calibrtion with parametes constrained by means of a Gaussian function and completely floating calibration parameters, to guarantee a stable and unbiased decay-time fit.
...
It should be noted that the calibration for the OS taggers is shifting the estimated mistags to higher values (\ie $p_1^{\text{\tiny OS}}>1$), while the calibration for the SS taggers does the opposite (\ie $p_1^{\text{\tiny SS}}<1$).
...
For all studies presented below then the pull distributions of the floating parameters are checked, whereby the pull is defined as the fitted value minus the value used in the generation of the simulated sample divided by the uncertainty of the fitted value.
...
\begin{itemize}
	\item In the first approach the tag decision is flipped in case the mistag probabiltity $\omega'$ exceeds \num{0.5} and the mistag probability is calculated as $\omega=1-\omega'$.
	...
	In the fit the tag decision is again flipped in case the mistag probabiltity $\omega'$ exceeds \num{0.5} and the mistag probability is calculated as $\omega=1-\omega'$.
...
\end{figure}
	\item In a last study the pseudoexeriments are again generated with artificially increased tagging asymmetry parameters.
...
On the other hand, reducing the allowed range of estimated mistags, prevents a bias on the measurement of \CP parameters, but depending on the cut which need to be applied, this could in principle reduce the statistical sensitivity of an analysis.
Therefore the modified \emph{link function} as used in the nominal approach provides currently the best appraoch as will be shown in \cref{sec:valOnSim}.

\subsection{Decay-time fits in subsamples}
\label{sec:valOnSubSample}

The stability of the fit is checked by performing the fit also in different sub samples of full dataset.
For this the dataset can be split several ways, namely by data taking conditions, used tagging algorithms or kinematic properties of the \Bz meson or properties of the event.
...
The obtained results for \Sf and \Sfbar show good agreement and the average of the results from the fits in the subsamples is well compatible with the result from the global fit.
\begin{figure}[tbp]
    \centering
    ...
    The blue points are the results of the fits in the subsamples, the red dashed area represents the result of the gloabl fit and the black line is the average of the results obtained in the subsamples.}
...
The first subsample contains candidates tagged exclusively by the OS algorithms while the second sample contains candidates which only got a tag assigned on the SS .
The third class then contains candidates which are tagged by both, the OS taggers and SS taggers.
...
Moreover, the difference in pseudo-rapidity is also sensitive to possible misalignments in the detector, which could have influence the measurement of the \Sf and \Sfbar.
...
Furthermore, the fit is validated using simulated events.
For this purpose the simulated events are bootstrapped, \ie the simulated data sample is resampled $n$ times, whereby it is allowed that single events can be taken more than once, \eg a bootstrapped sample can contain the same event multiple times.
This is statisitically valid because individual events are not correlated with each other.
...
Each sample is then fitted with the same strategy as the nominal fit extracting the \CP observables.
...
\end{figure}
Furthermore the following configurations are investigated also with \num{1000} bootstrapped samples each:
\begin{itemize}
	\item Using the true generated flavour of the \B candidate instead of the tag decision and mistag estimate provided by some tagging algorithm leads an unbiased distribution of residuals for \Sf and \Sfbar.
	...
	In this way the mistag is used as conditional observable as in the nominal fit, but still the truth information from the simulation is exploited.
	Also this appraoch gives unbiased results for \Sf and \Sfbar.
	...
	\item Instead of fixing the calibration parameters, obtained on simulated samples in the same way as described in \cref{sec:SScalibration} and \cref{sec:OScalibration}, in the decay-time fits to the  bootstrapped simulation samples of \BdToDpi they can be also constrained by means of Gaussian functions.
...
However, since the source of this small, potential bias cannot be narrowed down further than that it comes from the flavour tagging calibration, it is included as systematic uncertainty.
...
Finally, the average value from both studies, \ie \num{0.0068\pm0.0005} and \num{-0.0018\pm0.0005} for \Sf and \Sfbar, respectively, is assumed as systematic uncertainty in \cref{ch:systeamticUncerts}.
