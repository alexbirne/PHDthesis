...
The final acceptance parameterisation is then defined by the limits of the ranges on which the cubic polynomials are defined (also denoted as knots) and associated coefficients.
...
\end{figure}
	...
	But instead of flipping the tag decision it is to $d=0$ when the mistag probability exceeds \num{0.5}.
	Additionally, to achieve a stable fit the distribution of estimated mistags $\eta$ for the OS and SS is reduced beforehand to
	\begin{equation}
	...
	This assures that the mistag probabilities barely exceed \num{0.5}.
...
On the other hand, reducing the allowed range of estimated mistags prevents a bias on the measurement of \CP parameters, but depending on the cut which needs to be applied, this could reduce the statistical sensitivity of an analysis.
...
\subsection{Cross checks on subsamples}
